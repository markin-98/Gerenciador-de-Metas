...
export function GoalCard({ goal, savedCents, percent, style }: GoalCardProps) {
  ...
  return (
    <Link
      to={`/goals/${goal.id}`}
      style={style}
      className="animate-fade-in-up card-elevated card-elevated-hover flex flex-col gap-3 p-6 transition-transform active:scale-[0.98] active:ring-2 active:ring-primary"
    >
